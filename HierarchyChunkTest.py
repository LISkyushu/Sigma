...
class HierarchyChunkTest:
    ...
    def getInput(self, reset = False):
        ...
        if self.previous_output_class is None or np.array_equal(self.previous_output_class, self.output_class):
            ...
            #---------------input_label
        input_label=self.true_labels[self.output_class.argmax()]
        return input_value,input_label
    ...
    def getSequence(self, sequence_size):
        ...
        self.input_class = np.empty((sequence_size,self.true_labels.shape[1]))
        
        for i in range(sequence_size):
            
            input_value,input_label = self.getInput()
            #print("input_label",input_label)
            #input_class.append(self.chunk)
            #input_sequence.append(input_value)
            
            self.input_class[i] = input_label
